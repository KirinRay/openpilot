...
// family.
//
// 2026-08-28 CP11明文补充: 补入 GM 雷达保活帧 (0xA1/0x306/0x308/0x310 → bus1 MRR)。
// 产品版最初故意不含 GM 雷达保活, 但 VM 模拟 + 真机唐DM 的 "front radar error" 根因
// 正是缺 GM 雷达激活/保活帧。CP11 明文 safety_byd.h 的 BYD_TORQUE_TX_MSGS 含这4帧,
// 现对齐补入, 解决雷达报错。ANGLE_MODE/QINPLUS/SEAL 仍不引入 (避免回归)。
//
...
// GM radar activation/keepalive frames (for external GM radar on BYD, CP11明文)
#define BYD_CANADDR_GM_ADAS_TIME_STATUS     0x0A1
#define BYD_CANADDR_GM_ADAS_STEERING_STATUS 0x306
#define BYD_CANADDR_GM_ADAS_SPEED_STATUS    0x308
#define BYD_CANADDR_GM_ASCM_HEADLIGHT       0x310
...
static int byd_fwd_hook(CANPacket_t *to_send) {
  ...
  if (bus == BYD_CANBUS_ESC) {
    ...
    // 对齐 yysnet(汉验证通过): 不 block 0x32D (ACC_HUD_ADAS), 原车摄像头视觉帧须转发到 ESC
...
static safety_config byd_init(uint16_t param) {
  ...
  static const CanMsg BYD_TX_MSGS[] = {
    ...
    // GM radar activation / keepalive on MRR bus (CP11明文补充, 解决 front radar error)
    {BYD_CANADDR_GM_ADAS_TIME_STATUS,     BYD_CANBUS_MRR, 7},
    {BYD_CANADDR_GM_ADAS_STEERING_STATUS, BYD_CANBUS_MRR, 8},
    {BYD_CANADDR_GM_ADAS_SPEED_STATUS,    BYD_CANBUS_MRR, 7},
    {BYD_CANADDR_GM_ASCM_HEADLIGHT,       BYD_CANBUS_MRR, 2},
  };
